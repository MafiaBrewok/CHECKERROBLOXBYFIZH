...
def clean_cookie(cookie_string):
    cookie = cookie_string.strip()
    if ".ROBLOSECURITY=" in cookie:
        match = re.search(r'\.ROBLOSECURITY=([^;]+)', cookie)
        if match:
...
def get_roblox_headers(csrf_token=None):
    headers = {
...
def get_csrf_token(cookie_val):
    cookies = {'.ROBLOSECURITY': cookie_val}
    try:
        res = requests.post("https://auth.roblox.com/v2/logout", cookies=cookies, headers=get_roblox_headers())
        return res.headers.get("x-csrf-token", "")
    except Exception:
        return ""

def refresh_roblox_cookie(old_cookie_input):
    clean_c = clean_cookie(old_cookie_input)
    cookies = {'.ROBLOSECURITY': clean_c}
    csrf_token = get_csrf_token(clean_c)
    headers = get_roblox_headers(csrf_token)
...
def get_user_email_info(cookie_val, csrf_token):
    cookies = {'.ROBLOSECURITY': cookie_val}
    headers = get_roblox_headers(csrf_token)
    
    email_data = {"email": "Tidak dapat diakses / Tersembunyi", "verified": "Belum Terverifikasi"}
    try:
        # Endpoint resmi accountsettings untuk email
        res = requests.get("https://accountsettings.roblox.com/v1/email", cookies=cookies, headers=headers)
        if res.status_code == 200:
            data = res.json()
...
def get_user_inventory(user_id, cookie_val, csrf_token):
    cookies = {'.ROBLOSECURITY': cookie_val}
    headers = get_roblox_headers(csrf_token)
...
def get_user_game_history(user_id):
    history_maps = []
    try:
        res = requests.get(f"https://games.roblox.com/v1/users/{user_id}/games?limit=10")
        if res.status_code == 200:
            data = res.json().get("data", [])
            for g in data:
                game_name = g.get("name", "Unknown Game")
    ...
    # Fallback jika endpoint games kosong, coba ambil dari badges
    if not history_maps:
        try:
...
def get_user_spent_history(user_id, cookie_val, csrf_token):
    cookies = {'.ROBLOSECURITY': cookie_val}
    headers = get_roblox_headers(csrf_token)
    spent_maps = []
    total_spent_val = 0
    try:
        # Ambil transaksi tipe Purchases
        res = requests.get(f"https://economy.roblox.com/v1/users/{user_id}/transactions?transactionType=Purchases&limit=20", cookies=cookies, headers=headers)
        if res.status_code == 200:
            data = res.json().get("data", [])
            for t in data:
                item_name = t.get("details", {}).get("name", "Game Item / Gamepass")
                currency = t.get("currency", {})
                robux_spent = currency.get("amount", 0)
                cost = abs(robux_spent)
                total_spent_val += cost
                spent_maps.append({
                    "item": item_name,
                    "cost": cost
                })
        ...
        pass
    return spent_maps, total_spent_val

def check_roblox_account(cookie_input):
    cookie_val = clean_cookie(cookie_input)
    cookies = {'.ROBLOSECURITY': cookie_val}
    csrf_token = get_csrf_token(cookie_val)
    headers = get_roblox_headers(csrf_token)
    ...
    email_info = get_user_email_info(cookie_val, csrf_token)
    inventory_items, total_rap = get_user_inventory(user_id, cookie_val, csrf_token)
    history_maps = get_user_game_history(user_id)
    spent_maps, total_spent = get_user_spent_history(user_id, cookie_val, csrf_token)
